...
function buildChartData(strategyData: StrategyDataPoint[]): ChartPoint[] {
  ...
  return timelineDates.map((timelineDate) => {
    ...
    return {
      ...
      isTradeDay: dailyReturnPercent != null,
...
function sampleChartDataByPeriod(chartData: ChartPoint[], mode: Extract<ChartViewMode, "monthly" | "quarterly" | "yearly">): ChartPoint[] {
  ...
  const orderedChartData = [...chartData].sort(
    (left, right) => new Date(left.fullDate).getTime() - new Date(right.fullDate).getTime(),
  );
  const firstPointsByBucket = new Map<string, ChartPoint>();
  const firstTradePointsByBucket = new Map<string, ChartPoint>();
  const latestTradePointsByBucket = new Map<string, ChartPoint>();

  orderedChartData.forEach((point) => {
    ...
    if (!firstPointsByBucket.has(bucketKey)) {
      firstPointsByBucket.set(bucketKey, point);
    }

    // Monthly / quarterly / yearly points should anchor to the first real
    // trade day inside the period when one exists. If a period has no trade
    // day at all, fall back to the first available timeline day.
    if (point.isTradeDay && !firstTradePointsByBucket.has(bucketKey)) {
      firstTradePointsByBucket.set(bucketKey, point);
    }

    if (point.isTradeDay) {
      latestTradePointsByBucket.set(bucketKey, point);
    }
  });

  const activeBucketKey = (() => {
    const latestPoint = orderedChartData.at(-1);
    if (!latestPoint) return null;
    const latestDate = new Date(latestPoint.fullDate);
    return mode === "monthly"
      ? `${latestDate.getUTCFullYear()}-${String(latestDate.getUTCMonth() + 1).padStart(2, "0")}`
      : mode === "quarterly"
        ? `${latestDate.getUTCFullYear()}-Q${Math.floor(latestDate.getUTCMonth() / 3) + 1}`
        : `${latestDate.getUTCFullYear()}`;
  })();

  const sampled = Array.from(firstPointsByBucket.entries()).map(([bucketKey, fallbackPoint]) => {
    const firstTradePoint = firstTradePointsByBucket.get(bucketKey) ?? fallbackPoint;
    const latestTradePoint = latestTradePointsByBucket.get(bucketKey);

    const point =
      mode === "monthly"
      && bucketKey === activeBucketKey
      && latestTradePoint
      && latestTradePoint.curve1x > firstTradePoint.curve1x
        ? latestTradePoint
        : firstTradePoint;
    const date = new Date(point.fullDate);
    return {
  ...
  return [buildPeriodStartPoint(orderedChartData, mode), ...sampled];
}
